...
function main(): void {
  ...
  assertIncludes(formatCommand(windows), "--paths solver_cli", "solver source path");
  assertIncludes(formatCommand(windows), "--collect-all ortools", "ortools binaries collected");
  assertIncludes(formatCommand(windows), "solver_cli/shift_solver/veteran_rule_policy.json;shift_solver", "veteran policy bundled");
